...
module global
  ...
  REAL(8), public, parameter :: dt                       = 0.005 ! integration time step
  REAL(8), public, parameter  :: radius                   = 5.0 ! cell radius
  REAL(8), public, parameter  :: alpha_v                  = 1.0 ! lagrange multiplier coefficient
  REAL(8), public, parameter  :: epsilon                  = 1.0 ! surface tension

  !> some strings
  CHARACTER(len=:), allocatable, public :: string, dir_name, sim_id, format_string
  CHARACTER(len=20), public :: arg_iseed, arg_density, arg_chi, arg_gamma, arg_eta, arg_simid, arg_number_of_cells, file_name, file_id

  INTEGER, allocatable, public :: sphere(:,:), border_points(:), box_position(:,:)
  INTEGER, public :: ip, s(3), L(3), box_length(3), nstep, tsteps, iseed, output_counter, output_period, np_sphere, ip_global, nprint
